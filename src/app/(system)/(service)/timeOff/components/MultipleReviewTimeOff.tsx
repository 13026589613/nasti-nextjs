...
import useReturnTimeOffTableColumn from "../hooks/tableColumnReject";
import { TimeOffStatus, TimeOffVo } from "../types";
type MultipleReviewTimeOffDiaProps = {
  selectedRows: TimeOffVo[];
  type: TimeOffStatus;
  onClose: () => void;
  onSuccess: () => void;
};
...
const MultipleReviewTimeOffDia = (props: MultipleReviewTimeOffDiaProps) => {
  const { type, selectedRows, onClose, onSuccess } = props;
  const { UTCMoment } = useGlobalTime();
  const [dialogLoading] = useState(false);
  const [submitLoading, setSubmitLoading] = useState(false);
  const { columns } = useReturnTimeOffTableColumn();
  const {
    control,
    formState: { errors },
    watch,
  ...
  const handleConfirm = async (type: TimeOffStatus) => {
    ...
    try {
      const res = await checkTimeOff({
        timeOffId: selectedRows.map((item) => item.id),
        status: type,
        comment: comment,
      });
      if (res.code === 200) {
  ...
  const handleGetTimeOffRequestShift = async () => {
    const timeOffIds = selectedRows.map((item) => item.id).join(",");

    if (timeOffIds) {
      try {
        setSubmitLoading(true);

        const userTimeOffRes = await userTimeOffRequestHandleShift(timeOffIds);
  ...
  return (
    <Dialog
      open
      width="980px"
      title={type === "APPROVED" ? "Approve Time Off" : "Reject Time Off"}
      onClose={onClose}
    ...
    >
      <div className="overflow-y-auto overflow-x-hidden w-full max-h-[calc(100vh-180px)] pr-3">
        <div className={cn("max-h-[calc(100vh-390px)]")}>
          <CustomTable
            columns={columns}
            data={selectedRows}
            height="auto"
          ...
          <FormItem
            ...
            render={({ field: { value, onChange } }) => (
              <Textarea
                ...
                placeholder={
                  type === "APPROVED" ? "Approval Comment" : "Rejection Comment"
                }
              />
            )}
          />
      ...
      >
        <div className="mb-[15px]">
          The following employees have shifts scheduled during the requested
...
export default MultipleReviewTimeOffDia;
