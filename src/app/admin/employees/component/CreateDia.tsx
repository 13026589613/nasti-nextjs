...
const CreateDia = (props: CreateDiaProps) => {
  ...
  useEffect(() => {
    if (communityId && editItem) {
      getRoleList(communityId);
    }
  }, [communityId, editItem]);
  ...
  const addTypeDictinaryFn = async (data: any, type?: number | undefined) => {
    ...
    try {
      const res = await addEmployeeAdmin(data);
      if (res.code === 200) {
  ...
  const isShowInvite = useMemo(() => {
    return (
      !editItem || (currentBtn == "Pending" && editItem && editItem.status == 4)
    );
  }, [editItem, currentBtn]);
  ...
  return (
    ...
    >
      <CustomForm form={form} className="py-4 px-6" onSubmit={() => {}}>
        <>
        ...
        <div className="flex gap-6 justify-end">
          ...
          <CustomButton
            loading={loading}
            colorStyle={
              !editItem || (editItem && editItem.status == 4)
                ? "yellow"
                : undefined
            }
            type="submit"
