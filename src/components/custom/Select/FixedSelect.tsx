...
const FixedSelect = React.forwardRef<HTMLInputElement, CustomSelectProps>(
  (props, ref) => {
    const {
      options,
      dropdownRender,
    ...
    useLayoutEffect(() => {
      if (refs.current) {
        setSize(refs.current.controlRef.getBoundingClientRect());
      }
    }, [refs]);
...
FixedSelect.displayName = "FixedSelect";
  ...
  return (
    <div
      ...
      style={{
        ...
        top: height && placement === "top" ? `-${height + 10}px` : "0",
        marginTop: placement === "top" ? "0" : 10,
      }}
    >
      {children}
