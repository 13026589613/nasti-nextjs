...
export default function Select() {
    ...
    try {
      ...
      if (res.code === 200) {
        ...
        useTimeStore.getState().setGlobalTimeZone({
          globalTimeZone: communityInfo.zoneId,
          zoneAbbr: communityInfo.zoneShortName,
        });
        useTimeStore
          .getState()
          .setWeekOfStart(
  ...
  const departmentChange = (value: string | string[]) => {
    if (
      ...
      departmentInfo.isMultiple &&
      !departmentInfo.canEmpty
    ) {
    } else {
  ...
  return (
    <>
      <div className="flex-1 flex flex-col gap-5">
        <div className="flex flex-1 max-w-[400px] items-center gap-4 ml-2">
          <div className="flex-shrink-0 w-[84px] font-[390] text-[16px] text-[#919FB4] leading-10">
            Company
          </div>
          <CustomSelect
        ...
        {departmentInfo.isShow && (
          <div className="flex-1 max-w-[500px] flex items-center gap-4 ml-2">
            <div className="flex-shrink-0 w-[84px] font-[390] text-[16px] text-[#919FB4] leading-10">
              Department
            </div>
            <CustomSelect
              ...
              isClearable={CAN_EMPTY_DEPARTMENT_SELECT_PATH.includes(pathname)}
              isSearchable
